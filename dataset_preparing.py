...
def min_max_normalize(image, percentile, nodata=-1.0):
...
def non_max_suppression(coords, scores, threshold):
    ...
    while len(sorted_indices) > 0:
        ...
        distances = np.linalg.norm(coords[sorted_indices] -
...
def raster_to_polygons(raster, window, min_patch_objs=3, tolerance=1.2):
    ...
    if len(geoms) < min_patch_objs:
        ...
        geom_poly = [
            worldCoordPoly2CanvasPolygon(p,
                                         window_transform).simplify(tolerance)
            for p in polys
        ]
...
def get_coords_from_densifing_points(gdf, gap_distance=10, nms=False):
    ...
    origin_coordinates = np.concatenate(origin_coordinates,
                                        axis=0)  # .tolist()
    ...
    if nms:
        ...
        for i, row in p_gdf.iterrows():
            ...
            # int(gap_distance // 2)
            nms_idx = non_max_suppression(pos, coords_score,
                                          int(gap_distance // 2))
            ...
            nms_coords = np.concatenate([nms_coords, nms_coords[:1]],
                                        axis=0).tolist()
            if len(nms_coords) > 3:
...
def generate_heatmap(coords, shape, sigma=1):
    ...
    coords_value = max(np.min(blurred_heatmap[y_coords, x_coords]),
                       np.max(cpo))

    blurred_heatmap[y_coords, x_coords] = coords_value
    results_heatmap = min_max_normalize(blurred_heatmap[..., np.newaxis],
                                        percentile=0)
...
def run(rootdir, savedir, cfg_options, basename):
    ...
    raster_img = rasterio.open(
        os.path.join(os.path.join(rootdir, 'images'), basename))
    ...
    for sd in seed:
        ...
        gdf = raster_to_polygons(raster,
                                 window=window,
                                 min_patch_objs=min_patch_objs,
                                 tolerance=tolerance)
        if gdf is None:
            continue

        coords = get_coords_from_densifing_points(gdf,
...
if __name__ == '__main__':
    ...
    train_filelist = [x for x in filelist if not x in test_filelist]  # noqa

    # Train:
    train_savedir = '/nas/k8s/dev/research/doyoungi/dataset/Inria_building/train'  # noqa
    ...
    pool = Pool(4)
    for _ in tqdm(
            pool.imap_unordered(train_partial_function, train_filelist),
            total=len(train_filelist),
    ):
        pass
    pool.close()
    pool.join()
    ...
    pool = Pool(4)
    for _ in tqdm(
            pool.imap_unordered(test_partial_function, train_filelist),
            total=len(train_filelist),
    ):
        pass
    pool.close()
