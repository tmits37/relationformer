...
def scale_coordinates(coord, image_size, x0, y0):
    return int(coord[0] * image_size[0] + x0), int(coord[1] * image_size[1] +
                                                   y0)


def to_img_and_seg_from_results(filelist,
                                rootdir,
    ...
    for filename in filelist:
        ...
        # print(x0, y0)
        tot_img[x0 + p:x0 + patch_side - p,
                y0 + p:y0 + patch_side - p] = img[p:-p, p:-p]
        tot_seg[x0 + p:x0 + patch_side - p,
                y0 + p:y0 + patch_side - p] += seg[p:-p, p:-p, np.newaxis]
...
def to_graph_from_results(tot_img,
    ...
    for filename in filelist:
        ...
        #  save
        if single_patch:
            os.makedirs(os.path.join(rootdir, 'inference_plt', 'single_patch'),
                        exist_ok=True)
            blank_image = blank_image.astype('uint8')
            fig, ax = plt.subplots(1, 1, figsize=(9, 9))
            # fig, ax = plt.subplots(2, 1, figsize=(18,36))
            ax.imshow(blank_image[x0:x0 + patch_size[0],
                                  y0:y0 + patch_size[0]])
            savename = os.path.join(
                rootdir,
                'inference_plt',
                'single_patch',
                f'{region_num}_{x0}_{y0}_result.png',
            )
...
if __name__ == '__main__':
    ...
    rootdir = '/nas/k8s/dev/research/doyoungi/git/relationformer/show_dirs/road_rgb_2D'  # noqa
    ...
    if args.dataset == 'sat2graph':
        ...
        with open(
                '/nas/k8s/dev/research/doyoungi/git/relationformer/data/spacenet3/data_split.json',  # noqa
                'r',
        ) as file:
        ...
        patch_size = (args.patch_size, args.patch_size)
        region_num_list = [
            '-'.join(x.split('_')[1:]) for x in split_info['test']
        ]
    else:
    ...
    for region_num in tqdm(region_num_list):
        ...
        tot_seg, tot_img = to_img_and_seg_from_results(filelist_p_region,
                                                       rootdir, image_size,
                                                       patch_size)
        tot_graph = to_graph_from_results(
            tot_img,
            filelist_g_region,
            rootdir,
            patch_size,
        ...
        if not args.single_patch:
            gt_tot_graph = to_graph_from_results(
                tot_img,
                filelist_g_region,
                rootdir,
                patch_size,
            ...
            savename = os.path.join(rootdir, 'inference_plt',
                                    f'{region_num}_result.png')
            plt.savefig(savename, dpi=200, facecolor='#eeeeee')
