...
from dataset_preparing import (generate_heatmap,
                               get_coords_from_densifing_points)
...
def is_clockwise_three_points(points):
    ...
    raise ValueError(
        'All points are collinear or polygon has less than 3 points')
...
class CrowdAI(Dataset):
    ...
    Attributes:
        ...
        sigma (float): Standard deviation for Gaussian kernel used in heatmap generation. # noqa
    ...
    Args:
        ...
        gap_distance (int, optional): Gap distance for densifying points. Defaults to 20. # noqa
        sigma (float, optional): Sigma value for Gaussian blur in heatmap. Defaults to 1.5. # noqa
    """
    def __init__(self,
                 images_directory,
                 annotations_path,
        ...
        print(
            f'--Gap Distance: {self.gap_distance}',
            f'--Sigma: {self.sigma}',
            f'--nms: {self.nms}',
        )
    ...
    def prepare_annotations(self, img):
        ...
        Returns:
            GeoDataFrame: A GeoDataFrame containing the geometrical data of annotations. # noqa
        """
    ...
    def loadSample(self, idx):
        ...
        coords, gdf = get_coords_from_densifing_points(
            origin_gdf, gap_distance=self.gap_distance, nms=self.nms)  # [N, 2]
        heatmap = generate_heatmap(coords, image.shape[:2], sigma=self.sigma)

        counterclockwise_poly = [(Polygon(list(polygon.exterior.coords)[::-1])
                                  if polygon.exterior.is_ccw else polygon)
...
if __name__ == '__main__':
    img_dir = '/nas/tsgil/dataset/Inria_building/cocostyle/images'
    anno_path = '/nas/tsgil/dataset/Inria_building/cocostyle/annotation.json'
