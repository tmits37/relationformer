...
class Img2HeatmapDataLoader(Dataset):  # 데이터셋 인스턴스
    ...
    def __getitem__(self, idx):
        ...
        image_data = resize(image_data, (320, 320, 3),
                            anti_aliasing=True,
                            preserve_range=True)  # 폴리월드도 업샘플링함. 안하면 에러
        ...
        htm_data = resize(htm_data, (320, 320),
                          anti_aliasing=True,
                          preserve_range=True)
    ...
    if mode == 'train':
        ...
        data_dicts = [{
            ...
            'ann': ann_file
                      ...
                      for img_file, htm_file, ann_file in zip(
                          img_files, htm_files, ann_files)]
        ...
        if (config.DATA.DATASET == 'Inria'
                or config.DATA.DATASET == 'US20-road-network-2D'):
            random.seed(config.DATA.SEED)
            random.shuffle(data_dicts)
            train_split = int(split * len(data_dicts))
            train_files, val_files = data_dicts[:train_split], data_dicts[
                train_split:]
