...
def generate_directed_adjacency_matrix(mask_type, weight_mask, pairs_list,
                                       picked_nodes, k):
...
class HungarianMatcher(nn.Module):
    ...
    def forward(self, outputs, targets, config=None):
        ...
        if matcher == 'Hungarian':
            ...
            indices = []
            pred_marginal = torch.ones(1,
                                       num_queries).to(device)  # (1, pred_N)
            for i, c in enumerate(C.split(sizes, -1)):  # B만큼 반복
                ...
                for row in range(dims[1]):
                    ...
                    gt_idx = np.append(
                        gt_idx, np.full(len(max_proba_cols), row, dtype=int))
        ...
        masked = []
        if config is not None:
            mask_type = config.MODEL.ADJ_MAT_MASK
            weight_mask = config.MODEL.W_MASK
        else:
            mask_type, weight_mask = 'None', 1
        sample_edges = []
        k = outputs['pred_nodes'].size(
            1)  # number of pred_nodes, 현재는 nms의 결과 256개
        for b in range(bs):
            mapping = {
                j: i
            ...
            if k < n:  # 헝가리안 매쳐의 전제 조건에서 벗어나는 조건이긴 함 bs*q < n
                edge_maps = {edge[0]: edge[1]
                             for edge in edges}  # 정답 엣지 start_idx: end_idx
                for i, j in edges:
                    circle = set()
                    if i not in mapping:
            ...
            adj_mat_label, masked_mat = generate_directed_adjacency_matrix(
                mask_type, weight_mask, sample_edge, mapping, k)
