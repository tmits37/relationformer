...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, query_embed, pos_embeds):
        ...
        if self.two_stage:
            ...
            # self.decoder.num_layers = 4
            # output_memory.shape = [B, 340, D]
            # self.decoder.class_embed = None
            # When tryed self.decoder.class_embed(output_memory), its results shape is [B, 340, 2]
            # output_proposals's [B, 340, 4]

            enc_outputs_class = self.decoder.class_embed[self.decoder.num_layers](output_memory)

            enc_outputs_coord_unact = self.decoder.bbox_embed[self.decoder.num_layers](output_memory) + output_proposals # [:,:,:2]

            topk = self.two_stage_num_proposals
            topk_proposals = torch.topk(enc_outputs_class[..., 0], topk, dim=1)[1]
            topk_coords_unact = torch.gather(enc_outputs_coord_unact, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, enc_outputs_coord_unact.size(-1))) # [B, topk, 4]
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
                 dropout=0.1, activation="relu",
                 n_levels=4, n_heads=8, n_points=4, 
                 rln_attn=False, two_stage=False):
        super().__init__()
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios, pos_embeds,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                if tmp.shape[1] != reference_points.shape[1]:
                    tmp = tmp[:,:-1,:]
...
def build_deforamble_transformer(config):
    return DeformableTransformer(
        ...
        return_intermediate_dec=config.MODEL.DECODER.TWO_STAGE,
        num_feature_levels=config.MODEL.DECODER.NUM_FEATURE_LEVELS,
        two_stage=config.MODEL.DECODER.TWO_STAGE,
