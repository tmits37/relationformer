...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value):
        b = query.size(0)
        query, key, value = [
            l(x).view(b, self.dim, self.n_heads, -1)  # noqa
            for l, x in zip(self.proj, (query, key, value))  # noqa
        ]

        b, d, h, n = query.shape
        scores = torch.einsum('bdhn,bdhm->bhnm', query, key) / d**0.5
        attn = torch.einsum('bhnm,bdhm->bdhn',
                            torch.nn.functional.softmax(scores, dim=-1), value)

        return self.merge(attn.contiguous().view(b, self.dim * self.n_heads,
                                                 -1))
...
class AttentionalPropagation(nn.Module):
    def __init__(self, feature_dim: int, n_heads: int):  # 64, 4
        ...
        self.mlp = MultiLayerPerceptron(
            [feature_dim * 2, feature_dim * 2, feature_dim])  # 128, 128, 64
        nn.init.constant_(self.mlp[-1].bias, 0.0)

    def forward(self, x):
        message = self.attn(
            x, x, x)  # query, key, value, message is attention result
        return self.mlp(
            torch.cat([x, message], dim=1)
        )  # the vertex descriptor is updated. x(l+1) = MLP(cat(x(l),a(l)))
...
class AttentionalGNN(nn.Module):
    def __init__(self, feature_dim: int, num_layers: int):  # 64, 4
        ...
        self.layers = nn.ModuleList([
            AttentionalPropagation(feature_dim, 4) for _ in range(num_layers)
        ]  # 64, 4
                                    )  # 64+64 -> 64 -> 64+64-> 64... 4번 반복
    ...
    def forward(self, feat, graph):
        ...
        desc = self.conv_desc(
            feat)  # matching descriptor, 노드간 타당한 연결을 생성하는데 사용된다
        offset = self.conv_offset(feat).permute(
            0, 2, 1)  # positional offset, 노드 위치와 합쳐진다. p = p+a*t, -1~1 값을 가진다
        return desc, offset
...
class OptimalMatching(nn.Module):
    def __init__(self):
        ...
        # Modules
        self.scorenet1 = ScoreNet(self.descriptor_dim *
                                  2)  # 시계방향 permutation matrix
        self.scorenet2 = ScoreNet(self.descriptor_dim *
                                  2)  # 반시계방향 permutation matrix
    ...
    def predict(self, feature_map, graph):
        ...
        for b in range(B):  # 배치 사이즈만큼 반복하여 전체 디스크립터(sel_desc) 만들기
            ...
            if b == 0:
                ...
                sel_desc = torch.cat((sel_desc, b_desc.unsqueeze(0)),
                                     dim=0)  # 최종적으로 B, D, N (B, 64,256)

        # Multi-layer Transformer network.
        norm_graph = self.normalize_coordinates(
            graph, W,
            input='global')  # out: normalized coordinate system [-1, 1]
        sel_desc, offset = self.gnn(sel_desc,
                                    norm_graph)  # gnn 실행하여 매칭 디스크립터와 좌표 오프셋 뽑기

        # Correct points coordinates
        norm_graph = norm_graph + offset * self.correction_radius  # 좌표 refine
        graph = self.normalize_coordinates(
            norm_graph, W,
            input='normalized')  # out: global coordinate system [0, W]
