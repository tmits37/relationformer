...
class LaneGNN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.pose_encoder(node_feats.float()) # [N,2] -> [N, D]
        initial_x = x

        if edge_attr.size(1) == 0:
            edge_attr = torch.tensor([], dtype=torch.float32, device=device).reshape(1, 0)
        else:
            edge_attr = self.edge_encoder(edge_attr.float()) # [E, 4] -> [E x D_E1]
            map_attr = self.map_encoder(edge_img_feats) # [E, 64] -> [E x D_E2]
