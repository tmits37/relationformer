...
class RRCNN_block(nn.Module):
    def __init__(self, ch_in, ch_out, t=2):
        ...
        self.RCNN = nn.Sequential(Recurrent_block(ch_out, t=t),
                                  Recurrent_block(ch_out, t=t))
        self.Conv_1x1 = nn.Conv2d(ch_in,
                                  ch_out,
                                  kernel_size=1,
                                  stride=1,
                                  padding=0)
...
class NonMaxSuppression(nn.Module):
    def __init__(self, n_peaks=256):
        ...
        self.unfold = nn.Unfold(kernel_size=self.k,
                                padding=self.p,
                                stride=self.s)
    ...
    def sample_peaks(self, x):
        ...
        for b in range(B):
            x_b = x[b, 0]
            idx = torch.topk(x_b.flatten(),
                             self.n_peaks).indices  # 여기서 pred_node 순서가 생기는 듯
...
class Backbone(nn.Module):
    def __init__(self, encoder, detectionBranch, nms, config, device='cuda'):
        super().__init__()
    ...
    def forward(self, samples):
        ...
        return feature_map, vertices_detection_mask, vertices_positions
...
def build_backbone(config, **kwargs):
    ...
    model = Backbone(encoder, detectionBranch, nms, config, **kwargs)
    return model
