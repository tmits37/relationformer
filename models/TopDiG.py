...
from models.optimal_matching import OptimalMatching  # noqa
...
class TopDiG(nn.Module):
    ...
    def descriptor_extraction(self, feature_map, vertices_positions):
        ...
        # 배치 사이즈만큼 반복하여 전체 디스크립터(sel_desc) 만들기
        for b in range(B):
    ...
    def forward(self, samples):
        ...
        feature_map, vertices_detect_mask, vertices_positions = self.backbone(
            samples)
        visual_descriptor = self.descriptor_extraction(
            feature_map, vertices_positions[1])  # B, D, 256
        scores1, scores2 = self.decoder(vertices_positions[1],
                                        visual_descriptor)
        # scores1, scores2 = self.decoder.predict(feature_map, vertices_positions[1]) # 폴리월드 디코더 # noqa
        output = {
            'pred_nodes': (vertices_positions[1] / 320).to(self.device),
            'pred_heatmaps': vertices_detect_mask,
            'scores1': scores1,
            'scores2': scores2,
        }
...
def build_TopDiG(config, **kwargs):  # 일단 ptm 쓰는게 default인 상태
    pretrained_backbone = torch.load(config.DATA.BACKBONE_PATH,
                                     map_location='cpu')
    ...
    model = TopDiG(backbone, decoder, config, **kwargs)
