...
class Sinkhorn(torch.autograd.Function):
    """An implementation of a Sinkhorn layer with our custom backward module,
    based on implicit differentiation.

    :param c: input cost matrix, size [*,m,n], where * are arbitrarily many batch dimensions # noqa
    ...
    def backward(ctx, grad_p):
        ...
        t = torch.cat((grad_p.sum(dim=-1), grad_p[..., :, :-1].sum(dim=-2)),
                      dim=-1).unsqueeze(-1)
        ...
        grad_b = torch.cat(
            (
                grad_ab[..., m:, :],
                torch.zeros(
                    batch_shape + [1, 1], device=DEVICE, dtype=torch.float32),
            ),
            dim=-2,
        )
