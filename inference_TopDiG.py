...
def scores_to_permutations(scores):  # 인퍼런스용 함수
    ...
    for b in range(B):
        ...
        # 점수가 높을 수록 페어일 확률이 높으므로 -를 붙여서 최소 찾는 문제로 바꾼다.
        perm[b, r, c] = (
            1  # 헝가리안 알고리즘이 찾은 칸은 1로 아니면 0인 permu mat (B N N)
        )
    ...
    for cycle in cycles:
        if len(cycle) > 2:  # Polygon needs at least 3 points
            points = [G.nodes[node]['pos']
                      for node in cycle]  # Assuming 'pos' contains coordinates
...
def compute_metrics(pred_nodes, pred_edges, gt_nodes, gt_edges):
    ...
    rasterized_pred, rasterized_gt = rasterized_pred.flatten(
    ), rasterized_gt.flatten()
    rasterized_topo_pred, rasterized_topo_gt = (
        rasterized_topo_pred.flatten(),
        rasterized_topo_gt.flatten(),
    )
    ...
    topo_f1 = f1_score(rasterized_topo_pred,
                       rasterized_topo_gt,
                       average='macro')
    topo_miou = jaccard_score(rasterized_topo_pred,
                              rasterized_topo_gt,
                              average='macro')
    ...
    for idx, (images, heatmaps, nodes,
        ...
        for i in range(len(images)):
            ...
            acc, f1, miou, b_iou, topo_acc, topo_f1, topo_miou, topo_b_iou = (
                compute_metrics(out_nodes[i], pred_edges, nodes_i, edges_i))
            ...
            with open(metric_file, 'a') as file:
                file.write(
                    ...
                    f'b_iou: {b_iou}, topo_acc: {topo_acc}, '
